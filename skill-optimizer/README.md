...
在 Agent 中调用 `$skill-optimizer`；审计模式、授权边界和分级验证流程见 [SKILL.md](SKILL.md)。

小幅说明文字或元数据修改只做相关差异、引用和格式检查；行为、代码、安装或发行改动按影响补充验证。只有完整验收或相关判据要求时，才进行重新打包及新 Agent 真实任务验证。已有明确授权不重复确认，删除候选的完整确认链保持不变。
