...
把 Skill 当成可发行产品进行审计和优化，以“权威源码 → 发行载荷 → Agent 安装入口/软链 → Agent 实际加载 → 真实任务”的生命周期证据图，检查传播拓扑、版本坐标、死重候选、结构职责、依赖与交付闭环。
...
`release` 会自动启用 `review`。新增输出包括 `lifecycle`、`version_coordinates`、`runtime_verification`、`reachability` 和不含糊打分的 `structure` 矩阵。静态工具只产生 `deadweight-candidate`，不会确认删除；仅被 manifest 收录或来源标签提及不能证明运行使用。
...
详细判据见 [生命周期审计判据](references/lifecycle-audit.md)；Codex/RedSkill 差异见 [平台 Schema Profile](references/platform-schemas.md)。
