...
function markSlideReady(slide){
  ...
  var task=Promise.all([fonts].concat(images)).then(function(){slide.getBoundingClientRect();slide.dataset.renderReady='true';delete slide.dataset.renderError;updateDeckReady();return slide}).catch(function(error){markSlideError(slide,error);throw error});
  tasks.set(slide,task);return task;
}
function createEChart(slide,target,option){
  try{if(!global.echarts)throw new Error('ECharts 未加载');var element=typeof target==='string'?slide.querySelector(target):target;if(!element)throw new Error('找不到 ECharts 容器');var chart=global.echarts.init(element,null,{renderer:'svg'});chart.setOption(option);markSlideReady(slide);return chart}
  catch(error){markSlideError(slide,error);throw error}
}
global.WisePPT={markSlideReady:markSlideReady,markSlideError:markSlideError,createEChart:createEChart,updateDeckReady:updateDeckReady};

function initialize(){
  var root=document.documentElement,body=document.body,track=document.getElementById('track'),board=document.getElementById('board-sections'),pager=document.getElementById('pager'),deckStage=document.getElementById('deck-stage'),current=0,touchStartX=null;
  if(!track||!board||!deckStage)return;
  if(new URLSearchParams(location.search).get('print')==='1')root.classList.add('print-mode');
  ...
  document.addEventListener('wise-ppt:ready',function(){if(!root.classList.contains('print-mode'))rebuildBoard();if(new URLSearchParams(location.search).get('selftest')==='1')selfTest()},{once:true});
