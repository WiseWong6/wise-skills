...
> **优先级：① 自由绘制程序化 SVG（首选，图形化表达内容）→ ② layouts.md 版式骨架 → ③ atlas 组件改造（兜底）。**
> 图形化比表达组件重要：把关系画出来、把数字画出来；禁止"同构框+字+箭头"阵列充当图形。
> 本文件规定三件事：自由 SVG 的手法规约、外部库（FontAwesome/ECharts）的纸墨化、atlas 组件的改造五步。
