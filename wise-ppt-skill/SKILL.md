...
1. **单色**：全 deck 只有墨 `#191917` 及其透明度阶梯，纸底 `#DFE0D9`；无彩色、无渐变。强调靠线宽/字重/字族/虚实。
...
4. **可用空间双向居中**：内容主体必须在"剔除固定件后的可用矩形"内**水平 + 垂直双向居中**。可用矩形 = 左右边距之间（常规 x150–1770）× 纵向 y170–880（剔除左上角注区与底部 caption/folio 区）。多区块构图（如上矩阵 + 下 KPI 带）各区块中心也必须对齐到同一中点（x960）。**判定**：浏览器量内容包围盒 `getBBox()`，水平中心偏 x960 >10px、或垂直中心偏 y525 >30px 即不合格，整体平移修正。详见 design-tokens §4.4。
5. **Font Mixing（衬线 × 无衬线不串岗）**：标题/金句/大字=衬线气质族（statement=思源宋体 Medium，全 deck ≤3 页；引用/金句=霞鹜文楷大字）；正文/数据/UI/标签=无衬线族（思源黑体 Light=正文、Courier Prime=编号/数字/档案注）；手写批注=文楷（≤3 处）。全部本地开源可商用字体。
6. **视觉平衡 65/35**：内容（文字信息）≈65% + 视觉元素（图形/装饰）≈35%；留白即构图，不把留白当"没排满"去填。
7. **间距、阴影与边框**：所有 padding/margin/gap 落在 4pt 网格（design-tokens §4.7 档位表，优先用 `var(--space-N)`）；实心组件用两档阴影提升质感——`--shadow-soft`（卡片/面板标配，零偏移/墨色 2%/模糊 4px）或 `--shadow-specimen`（标本贴纸，2px 硬偏移/墨色 4%），**同层级不叠影**、色取边框墨色淡版（跟随主题）；**UI 组件边框（卡片/面板/气泡/按钮/chip/表格行/筛选框）一律 1px**，深浅靠颜色阶梯（墨 80%/50%/38%）拉开层级，不靠粗细——SVG 图形轮廓不在此列，保留多档线宽语义。
7. **三件套**：左上 doc 角注（mono 两行）、底部居中 caption 一句结论、左下 folio 页码署名（仅 `?ppt` 模式显示）。
8. **线宽档位**：主轮廓 1.2–1.4px @ 墨 80%，双线卡框内圈 0.6px，构造线 0.5–0.7px 虚线；强调线每页至多一处。
9. **数据自编**：UI mock、示例数据的名称与数值全部虚构，不搬用户真实业务数据。
10. **开放依赖（按需、纸墨化）**：可引入 FontAwesome 6（图标强制墨色、无彩色无动效）与 ECharts 5（仅复杂数据图表，必须去色改造为墨色阶梯；权威参考为官网 echarts.apache.org）。模板里放了注释掉的 CDN 链接，用时解开。
...
- **居中复核（硬规则 4，每页必做）**：目检时必须额外确认内容主体在可用空间内水平+垂直双向居中。对含复杂绝对定位坐标的版式（矩阵、多区块构图），用浏览器 `getBBox()` 量内容包围盒，水平中心偏 x960 >10px 即整体平移修正。
