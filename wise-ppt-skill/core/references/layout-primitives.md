...
原语不是 recipe 编号，而是可组合的空间规律。AI 先理解内容关系，再把唯一主原语写入 deck page 的 `spatial_primitive`，并先定义所需区域、阅读顺序、容量与组件角色，最后才查询公共 Gallery。查询后的布局来源决策只遵循 [`component-routing.md`](component-routing.md)。
...
需要总账时加 KPI / 结论带；需要解释细节时加 annotation；需要显式收口时加 converging path。辅助结构写进布局需求与选择理由。超过两个辅助结构通常意味着应拆页。
