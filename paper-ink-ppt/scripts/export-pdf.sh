#!/bin/bash
# 纸墨线稿 · 整 deck 导出 16:9 PDF
# 流程：frames 逐页 PNG（调 shot-screenshot.sh）→ 临时 print HTML（@page 20in×11.25in）
...
PNG_DIR="$(mktemp -d /tmp/paper-ink-pdf.XXXXXX)"
...
[ -n "$CHROME" ] || { echo "找不到 Chrome/Edge，请用 deck.html 手动打印为 PDF"; exit 1; }

# 1. 逐页 PNG
"$SKILL_ROOT/scripts/shot-screenshot.sh" "$DECK" "$PNG_DIR" >/dev/null || { echo "截图失败"; exit 1; }
...
# 5. 清理
[ "$KEEP_PNG" -eq 1 ] || rm -rf "$PNG_DIR"
[ "$KEEP_PNG" -eq 1 ] && echo "PNG 保留在 $PNG_DIR"
